...
async function doScan(
  ...
  await pMap(
    liqTargets,
    async (r) => {
      /* Мерится по deepBudgetLeft(), а не по budgetLeft(). budgetLeft вычитает
         DEEP_RESERVE_MS — резерв, заведённый для стадии стакана. Но стакан идёт
         ВЫШЕ (5a) и к этому месту уже отработал, а резерв продолжал удерживаться
         из бюджета стадии, которая идёт после него: резерв защищал то, что уже
         закончилось, и голодом морил то, что шло следом. Замер до правки: стадия
         отрабатывала 1 цикл из 12, поля ликвидаций и Л/Ш были пусты на всех 80
         строках в 9 циклах из 12, а в двух заполнялись частично — бюджет кончался
         на середине. После 5b сетевых стадий нет, резервировать не для кого. */
      if (deepBudgetLeft() <= 0) {
