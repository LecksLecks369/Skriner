...
/**
 * Исполнимый спред: нетто-разрыв минус проскальзывание ПОЛНОГО круга.
 *
 * Круг — ДВА пересечения обеих книг: вход (купить на одной, продать на другой) и
 * выход (обратная пара сделок). slipRoundTripPct — стоимость ОДНОГО такого
 * пересечения (сумма слипейджа обеих ног), поэтому из разрыва вычитается 2×.
 *
 * Живёт здесь, в модуле издержек, а не в score.ts, потому что выражение было
 * продублировано: лестница размеров в liquidity.ts вычитала 2×, а скалярное поле
 * строки — 1×, оба поля назывались netExecPct и ехали в одном объекте. На живом
 * скане расхождение было на всех 33 строках, где есть оба, и затвор алерта читал
 * оптимистичное: LSKUSDT давал +0.428% скаляром против −0.087% по лестнице.
 *
 * Результат НЕ обрезается снизу нулём. Обрезка делала 32 строки из 33 равными
 * ровно 0.000 — то есть уничтожала выборку, на которой расхождение было бы видно,
 * и оставляла различимой ровно ту строку, где оно решало. Отрицательное значение
 * здесь — осмысленный ответ «круг дороже разрыва», а не отсутствие данных;
 * отсутствие данных — это null.
 */
export function execSpreadPct(
  netSpreadPct: number | null,
  slipRoundTripPct: number | null | undefined
): number | null {
  if (netSpreadPct == null || slipRoundTripPct == null || !Number.isFinite(slipRoundTripPct)) return null;
  return Math.round((netSpreadPct - 2 * Math.max(0, slipRoundTripPct)) * 1000) / 1000;
}
