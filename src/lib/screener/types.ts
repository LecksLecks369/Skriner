...
export interface BreakoutSetup {
  ...
  /* Сигналом считается только готовность в пределах BREAKOUT_MAX_DIST_ATR от уровня
     (история замеров и почему порог именно такой — у самой константы в setups.ts).
     Поле ready несут и бейдж, и алерт, и журнал, чтобы три места не расходились
     в том, что считать сигналом. */
  ready: boolean;
  reasons: string[];
}
...
/* 'no_verdict' — улик прочитано меньше порога покрытия. Это ОТДЕЛЬНОЕ значение, а
   не 'тренд': «улики против» и «улик нет» разные утверждения, и пока второе
   называлось первым, отсутствие данных читалось как вывод о рынке. */
export type DistKind = 'pump_distribution' | 'dump_absorption' | 'pump_trend' | 'dump_trend' | 'no_verdict';

export interface DistributionState {
  kind: DistKind;
  dir: 'long' | 'short' | null; // сторона контр-сигнала; null у обычного тренда
  /* 0-100 — ДОЛЯ набранного от того, что можно было набрать при имеющихся данных,
     а не сумма баллов. Пока это была сумма, отсутствующее слагаемое молча голосовало
     против, и верхняя половина шкалы была недостижима: 85 записанных сигналов,
     максимум 60 из 100. */
  score: number;
  /* Какая доля веса улик вообще была прочитана (0..1). Решение по четверти улик и
     решение по всем уликам должны быть различимы, а не выглядеть как слабый и
     сильный сигнал одной природы. */
  coverage: number;
...
export interface ScanResponse {
  ...
  /* Сколько тикеров отброшено как не инструменты скринера: чужая квота (USDC/USD/USD1,
