...
export interface BreakoutSetup {
  ...
  /* Сигналом считается только готовность в пределах BREAKOUT_MAX_DIST_ATR от уровня.
     Замер на 50 исходах: дальше 0.5 ATR матожидание −0.284% с интервалом целиком ниже
     нуля (n=6 crypto; на полной выборке n=11 → −0.192% [−0.342; −0.057]), тогда как
     в пределах 0.5 ATR — +0.007%. Поле ready несут и бейдж, и алерт, и журнал, чтобы
     три места не расходились в том, что такое сигнал. */
  ready: boolean;
  reasons: string[];
}
