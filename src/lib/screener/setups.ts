...
/* Границы 95% ДИ того же замера. Контроль — ОЦЕНКА, а не константа: он снят на
   n=847, и у него своя неопределённость 6.70..10.44%. Сравнивать интервал детектора
   с точечным значением контроля значит приписывать контролю точность, которой у него
   нет, — и вердикт «выше базовой частоты» тогда выносится при перекрывающихся
   интервалах. Живой пример: детектор дал интервал от 9.0% против точки 8.4% и был
   объявлен положительным, хотя 9.0% лежит внутри 6.70..10.44%, то есть от контроля
   не отделён вовсе. Поэтому 'positive' требует превзойти ВЕРХНЮЮ границу контроля, а
   'negative' — уйти ниже НИЖНЕЙ: два интервала сравниваются как интервалы. */
export const CHOP_PERSIST_BASE_LO = 0.067;
export const CHOP_PERSIST_BASE_HI = 0.1044;
