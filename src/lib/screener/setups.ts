...
export const CHOP_ER_MAX = 0.025; // p10
/* Базовая частота исхода — прямое следствие того, что порог взят как p10: в 10%
   произвольных 30-минутных окон популяции эффективность и так оказывается ниже него.
   Это контроль, с которым обязан сравниваться win-rate ерша: попадание 10% означает
   ноль информации, а не «каждый десятый раз угадали». Двигаете CHOP_ER_MAX по
   квантилям — двигайте и эту константу на соответствующий процентиль. */
export const CHOP_BASE_RATE = 0.1;
