...
import { BREAKOUT_ALERT, BREAKOUT_MAX_DIST_ATR } from '@/lib/screener/setups';
...
const COLS: Array<{ key: string; label: string; title: string; cls?: string }> = [
  ...
  /* Подписи называют ЗАТВОР и РАБОЧИЙ ДИАПАЗОН, а не только шкалу. Шкала «0–100»,
     из которой на живых данных доступна четверть, читается как «78 — это много»,
     хотя это середина того, что вообще бывает: у готовности пробоя за всю историю
     записано 900 сигналов в полосе 60–88 при пороге 60, у силы раздачи — 85 в полосе
     45–60. Разброс внутри этих полос win-rate не различает (интервалы перекрываются
     целиком), поэтому число читается как «прошло затвор», а не как сила. */
  { key: 'breakout', label: 'Пробой', title: `Готовность к пробою: сжатие волатильности + прижатие к границе диапазона + набор OI + агрессия в сторону выхода. Стрелка — сторона уровня. Сигналом считается только цена в пределах ${BREAKOUT_MAX_DIST_ATR} ATR от уровня и ДО выхода за него: дальше цене до границы ещё идти, после выхода это уже не прогноз. В обоих случаях — прочерк. ЗАТВОР ${BREAKOUT_ALERT}: ниже сигнала нет. Наблюдаемый диапазон 60–88 из 100, и внутри него win-rate по полосам не различается — цифру стоит читать как «прошло затвор», а не как силу сигнала` },
  { key: 'chop', label: 'Ёрш', title: 'Пила: низкий коэффициент эффективности (Кауфман), свечи с длинными тенями, стопы снимают с обеих сторон. На готовность пробоя БОЛЬШЕ НЕ ВЛИЯЕТ: заявление «в пиле пробои чаще ложные» проверено на 541 исходе и не подтвердилось — полосы 0–14 → 38%, 45+ → 64%, интервалы перекрываются. Остаётся пометкой, пока полосы не разойдутся' },
  { key: 'dist', label: 'Раздача', title: 'Расхождение потока и цены внутри пампа/дампа: рост на падающем OI, продажи в рост, толпа набилась в ту же сторону. Контр-сигнал против движения. Цифра — ДОЛЯ набранного от того, что удалось прочитать (не сумма баллов): отсутствующая улика не голосует против. Затвор 45 при покрытии улик ≥50% — по четверти улик вердикт не выносится вовсе' },
